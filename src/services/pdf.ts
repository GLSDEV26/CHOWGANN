...
const GOLD = rgb(0.776, 0.655, 0.337)
...
export async function generateOrderPDF(order: Order, settings: Settings): Promise<Uint8Array> {
  ...
  page.drawText('CHOGAN', { x: margin, y: height - 55, size: 28, font: fontBold, color: GOLD })

  const bizName = settings.businessName || settings.ownerName
  if (bizName) {
    page.drawText(bizName, { x: margin, y: height - 72, size: 9, font: fontReg, color: rgb(0.6, 0.6, 0.6) })
  }

  page.drawText('Bon de commande', { x: width - margin - 120, y: height - 45, size: 11, font: fontBold, color: WHITE })
  page.drawText(order.orderNumber, { x: width - margin - 120, y: height - 62, size: 9, font: fontReg, color: GOLD })

  y = height - 100
  page.drawText(`Date : ${formatDate(order.createdAt)}`, { x: margin, y, size: 10, font: fontReg, color: GRAY })
  page.drawText(`Statut : ${STATUS_LABELS[order.status]}`, { x: width - margin - 160, y, size: 10, font: fontBold, color: GOLD })
  y -= 25
  page.drawText(`Paiement : ${PAYMENT_LABELS[order.paymentMethod]}`, { x: margin, y, size: 10, font: fontReg, color: GRAY })
  y -= 30
  ...
  const tableHeaders: [string, number][] = [
    ['Réf.', cols.ref], ['Désignation', cols.name], ['Qté', cols.qty], ['Rem.', cols.discount], ['Total', cols.total],
  ]
  tableHeaders.forEach(([label, x]) => {
    page.drawText(label, { x, y: y + 2, size: 8, font: fontBold, color: WHITE })
  })
  y -= 26
  ...
  const drawTotalLine = (label: string, value: string, bold = false) => {
    ...
    page.drawText(value, { x: valX - fontReg.widthOfTextAtSize(value, size), y, size, font, color: DARK })
    y -= bold ? 20 : 16
  }

  drawTotalLine('Sous-total', formatCurrency(order.subtotal))
  if (order.totalDiscount > 0) {
    drawTotalLine('Remise', `- ${formatCurrency(order.totalDiscount)}`)
  }
  page.drawLine({ start: { x: totalsX, y: y + 8 }, end: { x: width - margin, y: y + 8 }, thickness: 1, color: GOLD })
  y -= 5
  drawTotalLine('TOTAL', formatCurrency(order.totalAmount), true)
